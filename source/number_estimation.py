...
def estimate_numbers(N,
    ...
    for _ in range(individuals):
        ...
        # Add STM to Memory
        for i in range(len(stm_images)):
            img = create_image(stm_images[i], size, border_distance, minimum_distance, 'uniform', transform, radius, radius_distribution, radius_matrix, use_icon)
            memory_images.append(img)
        ...
        for n in rangy:
            ...
            # .. and its true quantity
            true_m = memory_true[closest_match]
            # ... and its label
            m = memory_labels[closest_match]
            # ... and offset
            prop_offset = true_m/m
            ...
            while (stop == False):
                ...
                if m in memory_labels and add_to_memory == True:
                    ...
                    # Find the index of the image in memory
                    index = memory_labels.index(m)
                    # Get the true quantity
                    true_m = memory_true[index]
                    prop_offset = true_m/m    
                    add_to_memory = False
                else:
                    # ... if not: Simulate a new image
                    img_m = round(m*prop_offset)
                    if img_m < 1:
                        img_m = 1
                    
                    compare_img = create_image(img_m, size, border_distance, minimum_distance,
                                               object_constellation, transform, radius, radius_distribution, radius_matrix, use_icon)
                    add_to_memory = True
                ...
                if decision_criterion:
                    ...
                    if (add_to_memory == True):
                        if (item_index == 99999):
                            memory_images.insert(0, control_img)
                            memory_labels.insert(0, m)
                            memory_true.insert(0, n)
                        else:
                ...
                if direction == 'up':
                    # Check if bouded
                    if (state == 'unbounded' and local_direction == 'down'):
                        state = 'bounded'
                ...
                if direction == 'down':
                    # Check if bouded
                    if (state == 'unbounded' and local_direction == 'up'):
                        state = 'bounded'
                    # Check if direction is changed
